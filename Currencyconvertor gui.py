...
def getting_API(user_currency, currency_to_be_converted):
    ...
    # Construct the URL with the access key and currencies as query parameters
    url = f'https://v6.exchangerate-api.com/v6/{access_key}/latest/{user_currency}'

    # Make the GET request
    r = requests.get(url)

    # Checking for request if it is unsuccessful
    ...
    # Check if both currencies are present in the fetched data
...
def convert_currency():
    ...
    # Check if user has entered a valid amount (only digits)
...
currencies = ['USD', 'EUR', 'GBP', 'JPY', 'CAD', 'PKR']  # Example currencies
